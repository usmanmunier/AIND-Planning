...
# Udacity Slack forum is awesome most of my problem got solved from there :)
class PgNode():
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            current_pg_node = PgNode_a(action)
            if current_pg_node.prenodes.issubset(self.s_levels[level]):
                new_a_nodes.append(current_pg_node)
                for s_node in self.s_levels[level]:
                   current_pg_node.parents.add(s_node)
                   s_node.children.add(current_pg_node)

        self.a_levels.append(new_a_nodes)
    ...
    def add_literal_level(self, level):
        ...
        """
        # I Was using array and forgot that TODO comments clearly says Set() should help here
        # Anyways was stuck on this badley then looked up and found that i have to use set
        # https://github.com/philferriere/aind-projects/blob/master/planning/
        # Above Repository helped me solve this.
        new_s_nodes=set()
        A_Nodes = self.a_levels[level - 1]
        for a_node in A_Nodes:
            for s_node in a_node.effnodes:
                new_s_nodes.add(s_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff_node1 in node_a1.effnodes:
            if eff_node1 in node_a2.prenodes:
                return True

        for eff_node2 in node_a2.effnodes:
            if eff_node2 in node_a1.prenodes:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        count = 0
        for parent_node1 in node_s1.parents:
            for parent_node2 in node_s2.parents:
                if parent_node1.is_mutex(parent_node2):
                    count +=1
        if count == len (node_s1.parents) and count == len (node_s2.parents):
            return True
    ...
    def h_levelsum(self) -> int:
        ...
        # Slack forum helped to fix my solution 
        for goal in self.problem.goal:
            node_s = PgNode_s(goal, True)
            for set in self.s_levels:
                for node in set:
                    if node_s.__eq__(node):
                        level_sum +=1
                        got_the_goal = True
                        break
